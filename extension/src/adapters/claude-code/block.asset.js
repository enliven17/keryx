...
(function () {
  "use strict";
  var TIER = __KERYX_TIER__;
  var AD = __KERYX_AD__;
  var ICON_REF = __KERYX_ICON__;
  var ICON_URL = __KERYX_ICON_URL__;
  var PORT = __KERYX_PORT__;
  var LBTOKEN = __KERYX_LBTOKEN__;
  var CLICKTOKEN = __KERYX_CLICKTOKEN__;
  ...
  var CLICKURL = __KERYX_CLICKURL__;
  ...
  var BANNER_ON = __KERYX_BANNER_ON__;
  ...
  var BASE = __KERYX_BASE__ || ("http://127.0.0.1:" + PORT + "/keryx/" + LBTOKEN);
  ...
  function buildAdHtml(tier, s) {
    ...
    var A1 = '<a href="' + href + '" target="_blank" ' +
      'rel="noopener noreferrer" data-keryx-ad="1" style="color:';
  ...
  // creative. data-keryx-ad="1" reuses the existing capture-phase click
  ...
  function buildBannerHtml(ad, clickUrl) {
    ...
    return '<span style="display:inline-flex;align-items:center;gap:6px;' +
      ...
      'data-keryx-ad="1" style="color:' + FG +
  ...
  try {
    ...
    // (baked into the block as __KERYX_VIEW_THRESHOLD_MS__; falls back
    // to 15_000 ms when the server didn't specify). Pure best-effort:
    // any throw is swallowed (prime directive).
    var THRESHOLD_MS = (typeof __KERYX_VIEW_THRESHOLD_MS__ === "number"
      && __KERYX_VIEW_THRESHOLD_MS__ > 0)
      ? __KERYX_VIEW_THRESHOLD_MS__ : 15000;
    var TICK_MS = 5000;
    ...
    document.addEventListener("click", function (ev) {
      ...
      while (el && el !== document) {
        if (el.getAttribute && el.getAttribute("data-keryx-ad")) {
          ...
          while (p && p !== document) {
            try {
              if (p.getAttribute && p.getAttribute("data-keryx-banner") === "1") {
                surface = "banner"; break;
              }
              if (p.getAttribute && p.getAttribute("data-keryx-overlay") === "1") {
    ...
    if (BANNER_ON) setInterval(function () {
      try {
        ...
        if (el.getAttribute("data-keryx-banner") === "1" &&
            _bannerLast === html) {
        ...
        el.innerHTML = html;
        el.setAttribute("data-keryx-banner", "1");
    ...
    // than strand it. While idle the ad keryxs NO view-time.
    ...
    function ensureOverlay(row) {
      ...
      overlay.setAttribute("data-keryx", String(TIER));
      overlay.setAttribute("data-keryx-overlay", "1");
    ...
    function retargetDockedOverlay() {
      try {
        var a = overlay && overlay.querySelector('a[data-keryx-ad]');
