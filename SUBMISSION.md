...
Every ad network on the internet settles on numbers its own server reports. The advertiser pays for impressions the network says it delivered; the publisher is paid whatever the network says it owes. Both parties trust the operator because no alternative is available to them. Fraud in digital advertising is measured in tens of billions of dollars a year, and it is structural: the party with the incentive to inflate the count is the party holding the count.
...
Visibility is not self-declared. The CLI's status-line renderer writes a heartbeat file on every Claude Code render, and the daemon only reports an impression while that heartbeat is fresh. If the terminal is not drawing the line, nothing accrues.
...
1. **Inclusion.** A Merkle proof that this transaction really is inside that block.
2. **Continuity.** That the block really belongs to the chain Creditcoin has attested, so a forged block cannot be substituted.
...
- `AttestcoinSettlement` (the ASC): [`0x42623b442fd0F3BC6796DA0a08a0074ba16f3209`](https://creditcoin-testnet.blockscout.com/address/0x42623b442fd0F3BC6796DA0a08a0074ba16f3209?tab=contract)
...
- `MockUSDC` (6 decimals): [`0x3ade9975CF961e4261d30fcF7C42BE7c71b522a9`](https://creditcoin-testnet.blockscout.com/address/0x3ade9975CF961e4261d30fcF7C42BE7c71b522a9?tab=contract)
...
`CampaignEscrow` holds campaign budgets and accrued balances. Advertisers call `createCampaign`, `fund`, `refund` and `closeCampaign`. Only addresses marked as *controllers* may call `setBid`, `charge` or `credit`. Earners call `claim` or `claimAll` and receive USDC directly, with no withdrawal approval and no operator in the path.
...
Enforced on-chain, where the operator cannot bypass them:

- **Recorder-gated emitter** (Sepolia contract): only the configured recorder can emit a source event.
- **Duplicate `receiptId` rejection** (Sepolia contract): one batch cannot be anchored twice.
- **Query id dedupe** (`ASCBase` on Creditcoin): one proof cannot be replayed.
- **Balance cap on charge** (Creditcoin contract): a campaign cannot be overdrawn.

Enforced by the operator, and stated as such rather than implied otherwise:

- **EIP-191 signature on every report:** stops one earner reporting as another.
- **Single-use nonce** (PostgreSQL): stops a captured report being replayed.
- **Five-minute timestamp window:** stops an old report being replayed.
- **Per-earner daily cap** (PostgreSQL): limits one earner farming impressions.
- **Render heartbeat** (client): stops reporting an ad that is not on screen.

See section 10 for what this split does and does not buy.
...
- Sepolia, `EngagementRecorded`: [`0x94d5993d…ae3e46`](https://sepolia.etherscan.io/tx/0x94d5993d3248bdf8363bfd66e3236bfa65190f6aec6d3c55ef614db755ae3e46)
- Creditcoin, `execute` then `EngagementSettled`: [`0xe75c826a…6b4c608`](https://creditcoin-testnet.blockscout.com/tx/0xe75c826a62b75b48caa813bef11b99ef00a3da3284181b9792a8e26996b4c608)

**Impression 2**

- Sepolia: [`0x358906be…f30cea0`](https://sepolia.etherscan.io/tx/0x358906be250533c0b7737fc9a7a34b220b4d556b5b11fbd41d0531998f30cea0)
- Creditcoin: [`0xf6b6464b…5d98994f`](https://creditcoin-testnet.blockscout.com/tx/0xf6b6464b57838c3745c8891c2952359447cb8e0824d634ac01477b865d98994f)

**Impression 3**

- Sepolia: [`0xa09fd018…0956127b`](https://sepolia.etherscan.io/tx/0xa09fd0180c590d2018c29374d499bc588520596f2695519c9463ee920956127b)
- Creditcoin: [`0x10202086…be3d108c`](https://creditcoin-testnet.blockscout.com/tx/0x10202086e0a73f0b37fa6924d2b179360db39bee42fa7c7f2815de80be3d108c)

On-chain result after the three: the earner's accrued balance went from `0` to `900` base units, and the campaign balance went from `100.000000` to `99.998200` USDC. Three impressions, 600 units charged each, split 50/50 every time.
...
- **`extension/`** (TypeScript, esbuild): VS Code surface. Patches the Claude Code bundle and restores it byte-for-byte on uninstall.
...
Hosting: the API and the Attestcoin worker run as one Railway service, and the dashboard is on Vercel. Both read the same Creditcoin deployment.

Tests: 11 Foundry tests, including the ASC's receipt decoding, foreign-emitter rejection, balance cap and revenue split. 17 extension unit tests. 46 Playwright end-to-end tests across desktop and mobile.
...
Deploying to testnet is two steps, the source emitter first and then the suite that points at it:
